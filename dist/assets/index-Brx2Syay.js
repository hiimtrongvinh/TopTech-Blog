(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))i(e);new MutationObserver(e=>{for(const c of e)if(c.type==="childList")for(const s of c.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function n(e){const c={};return e.integrity&&(c.integrity=e.integrity),e.referrerPolicy&&(c.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?c.credentials="include":e.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function i(e){if(e.ep)return;e.ep=!0;const c=n(e);fetch(e.href,c)}})();function M(t){return!t||t.startsWith("http")||t.startsWith("data:")?t:`/TopTech-Blog/${t.replace(/^\//,"")}`}function dt(t){t.innerHTML=`
  ...
  `}function ht(t){const o=document.getElementById("theme-toggle"),n=document.getElementById("menu-toggle"),i=document.getElementById("mega-menu"),e=document.getElementById("mega-menu-close"),c=document.getElementById("mega-menu-backdrop"),s=document.getElementById("search-trigger"),a=document.getElementById("search-overlay"),r=document.getElementById("search-close"),f=document.getElementById("search-input"),y=document.querySelectorAll(".nav-chuyen-muc-trigger");function E(){const p=document.querySelector(".sun-icon"),x=document.querySelector(".moon-icon");!p||!x||(document.body.classList.contains("dark")?(p.style.display="block",x.style.display="none"):(p.style.display="none",x.style.display="block"))}(localStorage.getItem("theme")||"light")==="dark"?document.body.classList.add("dark"):document.body.classList.remove("dark"),E(),o&&o.addEventListener("click",()=>{document.body.classList.contains("dark")?(document.body.classList.remove("dark"),localStorage.setItem("theme","light")):(document.body.classList.add("dark"),localStorage.setItem("theme","dark")),E()});const d=()=>i==null?void 0:i.classList.add("active"),v=()=>i==null?void 0:i.classList.remove("active");n==null||n.addEventListener("click",d),y.forEach(p=>p.addEventListener("click",d)),e==null||e.addEventListener("click",v),c==null||c.addEventListener("click",v);const g=i==null?void 0:i.querySelectorAll("a");g==null||g.forEach(p=>p.addEventListener("click",v)),s&&a&&s.addEventListener("click",()=>{a.classList.add("active"),setTimeout(()=>f==null?void 0:f.focus(),100)});const C=()=>{a==null||a.classList.remove("active"),f&&(f.value="")};r==null||r.addEventListener("click",C),a==null||a.addEventListener("click",p=>{p.target===a&&C()}),f==null||f.addEventListener("keydown",p=>{if(p.key==="Enter"&&t){const x=f.value.trim();x&&(C(),t(x))}})}function gt(t){const o=document.querySelector(".nav-home-link"),n=document.querySelector(".nav-blog-link"),i=document.querySelector(".write-post-btn");o==null||o.classList.remove("active"),n==null||n.classList.remove("active"),i==null||i.classList.remove("active"),t==="#/"||t===""||t.startsWith("#/ #")?o==null||o.classList.add("active"):t.startsWith("#/blog")||t.includes("bai-viet")?n==null||n.classList.add("active"):t.startsWith("#/admin")&&(i==null||i.classList.add("active"))}function mt(t){t.innerHTML=`
    <footer>
  ...
  `,pt(o),ut(o),vt(o),ft(o,n),yt(o)}function pt(t){const o=t.filter(f=>f.featured),n=o[0]||t.find(f=>f.id===1)||t[0];let i=o[1]||t.find(f=>f.id===2)||t[1];i&&n&&i.id===n.id&&(i=t.find(f=>f.id!==n.id)||t[1]);const e=document.getElementById("hero-banner-container");n&&e&&(e.innerHTML=`
    ...
    `);const s=document.getElementById("hero-side-container");let a=[];if(s){const f=[n==null?void 0:n.id,i==null?void 0:i.id].filter(Boolean);a=t.filter(y=>!f.includes(y.id)).slice(0,4),s.innerHTML=a.map(y=>`
      <div class="side-post-card">
        <img class="side-post-img" src="${M(y.image)}" alt="${y.title}">
        <div class="side-post-info">
          <h4 class="side-post-title"><a href="#/bai-viet/${y.id}">${y.title}</a></h4>
          <div class="post-meta" style="color: var(--text-muted); font-size: 0.75rem;">
            <span>${y.author} ${y.authorTag||""} &bull; ${y.date}</span>
          </div>
          <div class="post-meta" style="color: var(--text-muted); font-size: 0.75rem;">
            <span style="display: flex; align-items: center; gap: 0.2rem;"><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path><circle cx="12" cy="12" r="3"></circle></svg> ${y.views}</span>
            <span style="display: flex; align-items: center; gap: 0.2rem;"><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg> ${y.comments}</span>
          </div>
        </div>
      </div>
    `).join("")}const r=document.getElementById("hero-small-grid-container");if(r){const f=[n==null?void 0:n.id,i==null?void 0:i.id,...a.map(E=>E.id)].filter(Boolean),y=t.filter(E=>!f.includes(E.id)).slice(0,4);r.innerHTML=y.map(E=>`
  ...
  `).join("")}function vt(t){const o=document.getElementById("new-updates-content"),n=document.getElementById("col-center-ai"),i=document.getElementById("trending-content"),e=document.getElementById("reviews-content");if(o){const c=t[0]||t.find(r=>r.id===3),s=t[1]||t.find(r=>r.id===4),a=t[2]||t.find(r=>r.id===7);o.innerHTML=[c,s,a].filter(Boolean).map(r=>`
    ...
    `).join("")}if(n){const c=t.filter(f=>f.category==="AI"),s=c[0]||t.find(f=>f.id===5),a=c[1]||t.find(f=>f.id===6),r=c[2]||t.find(f=>f.id===8);n.innerHTML=`
      <!-- Main Center Post -->
    ...
    `).join("")}}function ft(t,o){const n=document.getElementById("category-columns-container");n&&(n.innerHTML=o.map((i,e)=>{const c=t.filter(a=>a.category===i),s=[];for(let a=0;a<4;a++)s.push(c[a%c.length]||t[(e*2+a)%t.length]);return`
  ...
  `;function z(){const g=document.getElementById("category-grid");if(!g)return;let C=[...c];r==="newest"?C.sort((m,b)=>b.id-m.id):r==="views"?C.sort((m,b)=>b.views-m.views):r==="comments"&&C.sort((m,b)=>b.comments-m.comments);const p=C.slice(0,f);g.innerHTML=p.length===0?`
    ...
    `).join("");const x=document.getElementById("load-more-container");x&&(C.length>f?x.style.display="block":x.style.display="none")}z();const d=document.getElementById("load-more-articles-btn");d&&d.addEventListener("click",()=>{f+=6,z()});const v=document.querySelectorAll(".filter-tab-btn");v.forEach(g=>{g.addEventListener("click",C=>{v.forEach(p=>p.classList.remove("active")),C.target.classList.add("active"),r=C.target.dataset.filter,f=6,z()})})}typeof window<"u"&&"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),"onvoiceschanged"in window.speechSynthesis&&(window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()}));function wt(t){if(!t)return"";if(t.trim().startsWith("<")||t.includes("</p>")||t.includes("</div>")||t.includes("</td>")||t.includes("</figure>"))return t;const o=t.split(`
`);let n=!1,i="",e=[];for(let s=0;s<o.length;s++){let a=o[s].trim();if(a.startsWith("|")&&a.endsWith("|")){n||(n=!0,i='<div class="table-responsive"><table class="post-detail-table">');const r=a.split("|").slice(1,-1).map(y=>y.trim());if(r.every(y=>/^:?-+:?$/.test(y)))continue;i.includes('<table class="post-detail-table">')&&!i.includes("<thead>")&&!i.includes("<tbody>")?i+="<thead><tr>"+r.map(y=>`<th>${y}</th>`).join("")+"</tr></thead><tbody>":i+="<tr>"+r.map(y=>`<td>${y}</td>`).join("")+"</tr>"}else n&&(n=!1,i+="</tbody></table></div>",e.push(i),i=""),e.push(o[s])}n&&(i+="</tbody></table></div>",e.push(i)),t=e.join(`
