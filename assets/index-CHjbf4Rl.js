(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))o(t);new MutationObserver(t=>{for(const c of t)if(c.type==="childList")for(const s of c.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function l(t){const c={};return t.integrity&&(c.integrity=t.integrity),t.referrerPolicy&&(c.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?c.credentials="include":t.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function o(t){if(t.ep)return;t.ep=!0;const c=l(t);fetch(t.href,c)}})();function z(e){return!e||e.startsWith("http")||e.startsWith("data:")?e:`/TopTech-Blog/${e.replace(/^\//,"")}`}function Et(e){e.innerHTML=`
    <header>
  ...
  `;let o=0;function t(a){const n=i[a%i.length],y=[i[(a+1)%i.length],i[(a+2)%i.length],i[(a+3)%i.length],i[(a+4)%i.length]],E=document.getElementById("hero-left-container");n&&E&&(E.innerHTML=`
      ...
      `).join(""))}t(o);const c=setInterval(()=>{o=(o+1)%i.length;const a=document.getElementById("hero-left-container"),n=document.getElementById("hero-side-container");a&&n&&(a.style.opacity="0.1",n.style.opacity="0.1",setTimeout(()=>{t(o),a.style.opacity="1",n.style.opacity="1"},300))},5e3),s=document.getElementById("hero-small-grid-container");if(s){const a=i.slice(5,9);s.innerHTML=a.map(n=>`
    ...
    `).join("")}At(i),Bt(i),qt(i,l),Ht(i),e.cleanup=()=>{clearInterval(c)}}function At(e){const i=document.getElementById("featured-numbers-container");if(!i)return;const l=e.filter(t=>t.featured)[0]||e.find(t=>t.id===1),o=e.filter(t=>t.id!==(l==null?void 0:l.id)).slice(0,6);i.innerHTML=o.map((t,c)=>`
  ...
  `).join("")}function Bt(e){const i=document.getElementById("new-updates-content"),l=document.getElementById("col-center-ai"),o=document.getElementById("trending-content"),t=document.getElementById("reviews-content");if(i){const c=e[0]||e.find(n=>n.id===3),s=e[1]||e.find(n=>n.id===4),a=e[2]||e.find(n=>n.id===7);i.innerHTML=[c,s,a].filter(Boolean).map(n=>`
    ...
    `).join("")}if(l){const c=e.filter(y=>y.category==="AI"),s=c[0]||e.find(y=>y.id===5),a=c[1]||e.find(y=>y.id===6),n=c[2]||e.find(y=>y.id===8);l.innerHTML=`
      <!-- Main Center Post -->
    ...
    `}if(o){const c=[...e].sort((s,a)=>(a.views||0)-(s.views||0)).slice(0,5);o.innerHTML=c.map(s=>`
    ...
    `).join("")}if(t){const c=e.filter(n=>n.category==="Review"),s=e.filter(n=>n.category!=="Review"),a=[...c,...s].slice(0,4);t.innerHTML=a.map(n=>`
    ...
    `).join("")}}function qt(e,i){const l=document.getElementById("category-columns-container");l&&(l.innerHTML=i.map((o,t)=>{const c=e.filter(a=>a.category===o),s=[];for(let a=0;a<4;a++)s.push(c[a%c.length]||e[(t*2+a)%e.length]);return`
    ...
    `}).join(""))}function Ht(e){const i=document.getElementById("refresh-new-updates"),l=document.getElementById("new-updates-content");i&&i.addEventListener("click",()=>{i.classList.add("spinning"),setTimeout(()=>{i.classList.remove("spinning");const o=[...e].filter(a=>a.id!==5&&a.id!==6).sort(()=>.5-Math.random()),t=o[0],c=o[1],s=o[2];l&&(l.innerHTML=[t,c,s].map(a=>`
            <div class="post-card-mini">
              <img class="post-card-mini-img" src="${z(a.image||"https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&q=80&w=400&h=250")}" alt="${a.title}">
              <div class="post-card-mini-info">
  ...
  `;function H(){const x=document.getElementById("category-grid");if(!x)return;let I=[...c];n==="newest"?I.sort((k,F)=>F.id-k.id):n==="views"?I.sort((k,F)=>F.views-k.views):n==="comments"&&I.sort((k,F)=>F.comments-k.comments);const P=I.slice(0,y);x.innerHTML=P.length===0?`
...
`);let l=!1,o="",t=[];for(let s=0;s<i.length;s++){let a=i[s].trim();if(a.startsWith("|")&&a.endsWith("|")){l||(l=!0,o='<div class="table-responsive"><table class="post-detail-table">');const n=a.split("|").slice(1,-1).map(E=>E.trim());if(n.every(E=>/^:?-+:?$/.test(E)))continue;o.includes('<table class="post-detail-table">')&&!o.includes("<thead>")&&!o.includes("<tbody>")?o+="<thead><tr>"+n.map(E=>`<th>${E}</th>`).join("")+"</tr></thead><tbody>":o+="<tr>"+n.map(E=>`<td>${E}</td>`).join("")+"</tr>"}else l&&(l=!1,o+="</tbody></table></div>",t.push(o),o=""),t.push(i[s])}l&&(o+="</tbody></table></div>",t.push(o)),e=t.join(`
`);let c=e.replace(/^### (.*$)/gim,'<h3 class="post-subheading-3">$1</h3>').replace(/^## (.*$)/gim,'<h2 class="post-subheading-2">$1</h2>').replace(/^# (.*$)/gim,'<h1 class="post-heading-1">$1</h1>').replace(/\!\[(.*?)\]\((.*?)\)/g,'<figure class="post-inline-figure"><img src="$2" alt="$1"><figcaption class="post-figcaption">$1</figcaption></figure>').replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/^\> (.*$)/gim,'<blockquote class="post-blockquote">$1</blockquote>').replace(/^\- (.*$)/gim,"<li>$1</li>").replace(/\`\`\`([\s\S]*?)\`\`\`/g,'<pre class="post-code-block"><code>$1</code></pre>').replace(/\`(.*?)\`/g,'<code class="post-inline-code">$1</code>').replace(/\n\n/g,"</p><p>");return c=c.replace(/(<li>.*<\/li>)/gim,"<ul>$1</ul>"),`<p>${c}</p>`}function zt(e,i,l){const o=parseInt(i,10),t=l.find(g=>g.id===o)||l[0];t.views+=1,localStorage.setItem("toptech_articles",JSON.stringify(l));const c=l.filter(g=>g.category===t.category&&g.id!==t.id).slice(0,4);t.commentsList||(t.commentsList=[{id:1,author:"Nguyễn Văn Hùng",date:"2 ngày trước",content:"Bài viết rất chi tiết và dễ hiểu! Tôi đang tìm hiểu về vấn đề này và thấy thông tin cực kỳ hữu ích cho công việc thực tế của mình.",avatar:"https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=80&h=80"},{id:2,author:"Lê Minh Hạnh",date:"1 ngày trước",content:"TopTech chia sẻ kiến thức chuẩn quá, thiết kế giao diện bài viết đọc rất sướng mắt. Mong nhóm tác giả ra thêm nhiều bài viết sâu hơn nữa!",avatar:"https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=80&h=80"}]),e.innerHTML=`
  ...
  `;function s(){const g=document.getElementById("comments-list");if(!g)return;g.innerHTML=t.commentsList.map(L=>`
  ...
  `;const t=document.getElementById("admin-password"),c=document.getElementById("login-submit-btn"),s=document.getElementById("login-error-msg"),a=()=>{(t==null?void 0:t.value.trim())==="admin123"?(sessionStorage.setItem("toptech_admin_logged_in","true"),vt(e,i,l,o)):(s&&(s.textContent="Mật khẩu không chính xác. Thử lại!",s.style.display="block"),t&&(t.style.borderColor="var(--secondary-color)"))};c==null||c.addEventListener("click",a),t==null||t.addEventListener("keydown",n=>{n.key==="Enter"&&a()})}function Rt(e){if(!e)return"";if(e.trim().startsWith("<")||e.includes("</p>")||e.includes("</div>")||e.includes("</td>")||e.includes("</figure>"))return e;const i=e.split(`
`);let l=!1,o="",t=[];for(let s=0;s<i.length;s++){let a=i[s].trim();if(a.startsWith("|")&&a.endsWith("|")){l||(l=!0,o='<div class="table-responsive"><table class="post-detail-table">');const n=a.split("|").slice(1,-1).map(E=>E.trim());if(n.every(E=>/^:?-+:?$/.test(E)))continue;o.includes('<table class="post-detail-table">')&&!o.includes("<thead>")&&!o.includes("<tbody>")?o+="<thead><tr>"+n.map(E=>`<th>${E}</th>`).join("")+"</tr></thead><tbody>":o+="<tr>"+n.map(E=>`<td>${E}</td>`).join("")+"</tr>"}else l&&(l=!1,o+="</tbody></table></div>",t.push(o),o=""),t.push(i[s])}l&&(o+="</tbody></table></div>",t.push(o)),e=t.join(`
`);let c=e.replace(/^### (.*$)/gim,'<h3 class="post-subheading-3">$1</h3>').replace(/^## (.*$)/gim,'<h2 class="post-subheading-2">$1</h2>').replace(/^# (.*$)/gim,'<h1 class="post-heading-1">$1</h1>').replace(/\!\[(.*?)\]\((.*?)\)/g,'<figure class="post-inline-figure"><img src="$2" alt="$1"><figcaption class="post-figcaption">$1</figcaption></figure>').replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/^\> (.*$)/gim,'<blockquote class="post-blockquote">$1</blockquote>').replace(/^\- (.*$)/gim,"<li>$1</li>").replace(/\`\`\`([\s\S]*?)\`\`\?/g,'<pre class="post-code-block"><code>$1</code></pre>').replace(/\`(.*?)\`/g,'<code class="post-inline-code">$1</code>').replace(/\n\n/g,"</p><p>");return c=c.replace(/(<li>.*<\/li>)/gim,"<ul>$1</ul>"),`<p>${c}</p>`}function Dt(e,i,l,o){let t="articles",c="list",s=null,a=$t[0].url,n=null,y=null;const E=sessionStorage.getItem("toptech_editing_article_id");if(E){const r=parseInt(E,10),d=i.find(p=>p.id===r);d&&(t="articles",c="write",s=r,a=d.image),sessionStorage.removeItem("toptech_editing_article_id")}function H(){const r=localStorage.getItem("toptech_categories");return r?JSON.parse(r):l}function q(r){localStorage.setItem("toptech_categories",JSON.stringify(r))}function N(){const r=localStorage.getItem("toptech_subcategories");return r?JSON.parse(r):{"Tin công nghệ":["Điện thoại","Máy tính","Thiết bị thông minh","Xe điện"],AI:["Machine Learning","Generative AI","ChatGPT & LLMs","Thị giác máy tính"],"Chuyển đổi số":["Doanh nghiệp","Giải pháp đám mây","IoT & Automation","Bảo mật dữ liệu"],Review:["Đánh giá Laptop","Đánh giá Smartphone","Đồ công nghệ khác","Trải nghiệm dịch vụ"],"Phần mềm":["Công cụ lập trình","Phần mềm văn phòng","Hệ điều hành","Ứng dụng di động"],IT:["Lập trình","Quản trị mạng","Cơ sở dữ liệu","DevOps"],"Giải pháp công nghệ":["AI cho doanh nghiệp","Tự động hóa quy trình","Hạ tầng số","ERP & CRM"],Casestudy:["Thành công tiêu biểu","Phân tích thất bại","Khảo sát thực tế"]}}function x(r){localStorage.setItem("toptech_subcategories",JSON.stringify(r))}function I(){const r=localStorage.getItem("toptech_authors");return r?JSON.parse(r):[{name:"Phương Linh",role:"Content Writer",desc:"Ms. Phương Linh là cây bút cốt cán tại TopTech chuyên viết về Trí tuệ nhân tạo (AI)...",avatar:"https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=150&h=150"},{name:"Ngọc Thanh",role:"Content Writer",desc:"Ms. Ngọc Thanh chuyên phân tích và đánh giá chuyên sâu các thiết bị di động...",avatar:"https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=150&h=150"},{name:"Lê Mạnh",role:"CEO & Founder",desc:"Mr. Lê Mạnh là nhà sáng lập kiêm Giám đốc điều hành của TopTech...",avatar:"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=150&h=150"},{name:"Kim Phượng",role:"Content Writer",desc:"Ms. Kim Phượng đam mê nghiên cứu ứng dụng phần mềm văn phòng...",avatar:"https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&q=80&w=150&h=150"},{name:"Trà My",role:"Software Developer",desc:"Ms. Trà My là kỹ sư phát triển phần mềm chuyên nghiệp...",avatar:"https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=150&h=150"}]}function P(r){localStorage.setItem("toptech_authors",JSON.stringify(r))}function v(){var r,d,p,m;e.innerHTML=`
    ...
    `,(r=document.getElementById("admin-logout-btn"))==null||r.addEventListener("click",()=>{sessionStorage.removeItem("toptech_admin_logged_in"),vt(e,i,l,o)}),(d=document.getElementById("tab-articles-btn"))==null||d.addEventListener("click",()=>{t="articles",c="list",v()}),(p=document.getElementById("tab-categories-btn"))==null||p.addEventListener("click",()=>{t="categories",n=null,v()}),(m=document.getElementById("tab-authors-btn"))==null||m.addEventListener("click",()=>{t="authors",y=null,v()}),t==="articles"?c==="list"?F():M():t==="categories"?J():t==="authors"&&ct()}function k(){if(c==="list")return`
          ...
          <div class="table-responsive">
            <table class="admin-table">
              ...
              <tbody>
                ...
                `:i.map(r=>`
                  <tr id="article-row-${r.id}">
                    <td>
                      <img class="table-post-thumb" src="${r.image.startsWith("http")||r.image.startsWith("data:")?r.image:z(r.image)}" alt="Thumbnail">
                    </td>
                    <td>
                      <div style="font-weight: 600; line-height: 1.4;">
                        <a href="#/bai-viet/${r.id}" target="_blank" class="table-post-title-link">${r.title}</a>
                      </div>
                      ${r.featured?'<span class="badge badge-featured">Nổi bật</span>':""}
                    </td>
                    <td>
                      <span class="badge badge-cat">${r.category}${r.subcategory?` / ${r.subcategory}`:""}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div style="font-size: 0.85rem;">
                        👀 <strong>${r.views||0}</strong> lượt xem
                      </div>
                      <div style="font-size: 0.85rem;">
                        💬 <strong>${r.comments||0}</strong> bình luận
                      </div>
                    </td>
                    <td>
                      <button class="edit-post-btn" data-id="${r.id}" style="margin-right: 0.5rem; background-color: var(--primary-color); border-color: var(--primary-color); color: #fff; padding: 0.4rem 0.8rem; border-radius: 6px; border: none; font-size: 0.75rem; cursor: pointer; display: inline-flex; align-items: center; gap: 2px;">
                        Sửa
                      </button>
                      <button class="delete-post-btn" data-id="${r.id}">
                        Xóa
                      </button>
                    </td>
                  </tr>
      ...
      `;{const r=s?i.find(m=>m.id===s):null,d=H(),p=I();return`
        ...
        <div class="create-post-layout-grid">
          <div class="publish-form-panel glass-panel">
            <h3 class="panel-subtitle">${r?"Chỉnh sửa bài viết":"Nội dung bài viết mới"}</h3>
            ...
            <div class="login-form" style="margin-top: 1.5rem;">
              <div class="form-row">
                ...
                <input type="text" id="post-title" value="${r?r.title:""}" placeholder="Nhập tiêu đề..." required>
              </div>
              ...
              <div class="form-grid-3">
                <div class="form-row">
                  ...
                  <select id="post-category" required>
                    ${d.map(m=>`<option value="${m}" ${r&&r.category===m?"selected":""}>${m}</option>`).join("")}
                  </select>
                </div>
                ...
                <div class="form-row" style="display:flex; align-items:center; gap: 0.5rem; margin-top: 1.8rem; cursor:pointer;">
                  <input type="checkbox" id="post-featured" ${r&&r.featured?"checked":""} style="width: 20px; height: 20px; margin:0;">
              ...
              <div class="form-row">
                ...
                <select id="post-author-name" required>
                  ${p.map(m=>`<option value="${m.name}" ${r&&r.author===m.name?"selected":""}>${m.name} (${m.role})</option>`).join("")}
                </select>
              </div>
              ...
              <div class="form-row" style="margin-top: 1rem;">
                ...
                <div style="margin-top: 0.8rem; display: flex; align-items: center; gap: 0.5rem;">
                  ...
                  <input type="url" id="post-cover-url" value="${r&&r.image.startsWith("http")?r.image:""}" placeholder="Dán link ảnh bìa (https://...)" style="margin: 0; padding: 0.5rem 0.8rem; font-size: 0.9rem;">
                </div>
              </div>

              <div class="form-row">
                <label for="post-description">Tóm tắt bài viết (Sapo) <span style="color:var(--secondary-color)">*</span></label>
                <textarea id="post-description" rows="3" placeholder="Viết tóm tắt ngắn..." required>${r?r.description:""}</textarea>
              </div>
              ...
              <button class="submit-comment-btn" id="publish-post-btn" style="margin-top: 1.5rem; width:100%; font-size: 1rem; padding: 0.8rem 1.5rem; background-color: var(--primary-color); color:#fff; border-color: var(--primary-color); border-radius: 30px;">
                ${r?"💾 Cập nhật bài viết":"🚀 Xuất bản bài viết"}
              </button>
            </div>
          </div>
          ...
          <div class="preview-sidebar-panel">
            ...
            <div class="cat-article-card" style="box-shadow: var(--shadow-lg); background-color: var(--bg-surface); border: 1px solid var(--border-color); pointer-events: none;">
              <div class="cat-card-img-wrapper">
                <img id="sidebar-preview-img" src="${a}" alt="Cover Preview" style="height: 200px; width: 100%; object-fit: cover;">
      ...
      `}}function F(){var r;(r=document.getElementById("btn-goto-write-article"))==null||r.addEventListener("click",()=>{c="write",s=null,a=$t[0].url,v()}),e.querySelectorAll(".edit-post-btn").forEach(d=>{d.addEventListener("click",()=>{const p=parseInt(d.dataset.id,10),m=i.find(f=>f.id===p);m&&(s=p,a=m.image,c="write",v())})}),e.querySelectorAll(".delete-post-btn").forEach(d=>{d.addEventListener("click",()=>{const p=parseInt(d.dataset.id,10),m=i.find(f=>f.id===p);if(confirm(`Bạn có chắc chắn muốn xóa bài viết "${m==null?void 0:m.title}"?`)){const f=i.filter(u=>u.id!==p);localStorage.setItem("toptech_articles",JSON.stringify(f));const T=document.getElementById(`article-row-${p}`);T&&(T.style.transition="opacity 0.3s ease",T.style.opacity="0",setTimeout(()=>{i.splice(0,i.length,...f),o(),v()},300))}})})}function M(){var dt;(dt=document.getElementById("btn-back-to-articles"))==null||dt.addEventListener("click",()=>{c="list",s=null,v()});const r=document.getElementById("post-title"),d=document.getElementById("post-category"),p=document.getElementById("post-subcategory"),m=document.getElementById("post-featured"),f=document.getElementById("post-author-name"),T=document.getElementById("post-description"),u=document.getElementById("post-content"),C=document.getElementById("publish-post-btn"),R=document.getElementById("sidebar-preview-img"),V=document.getElementById("sidebar-preview-cat"),g=document.getElementById("sidebar-preview-title"),b=document.getElementById("sidebar-preview-desc"),L=document.getElementById("sidebar-preview-author"),A=document.getElementById("sidebar-preview-time"),_=N(),O=s?i.find(h=>h.id===s):null;if(O&&u){u.innerHTML=O.content,a=O.image,R&&(R.src=a),V&&(V.textContent=(O.subcategory||O.category).toUpperCase()),g&&(g.textContent=O.title),b&&(b.textContent=O.description),L&&(L.textContent=O.author);const h=document.getElementById("upload-dropzone");h&&(h.style.borderColor="var(--primary-color)",h.innerHTML=`
        ...
        `;et("insertHTML",S)},$.readAsDataURL(h)};at==null||at.addEventListener("click",()=>X==null?void 0:X.click()),X==null||X.addEventListener("change",h=>D(h.target.files[0])),u==null||u.addEventListener("paste",h=>{var B;const $=(B=h.clipboardData)==null?void 0:B.items;if($){for(let S=0;S<$.length;S++)if($[S].type.indexOf("image")!==-1){h.preventDefault(),D($[S].getAsFile());break}}});const w=document.getElementById("upload-dropzone"),j=document.getElementById("cover-file-input"),Y=document.getElementById("post-cover-url");w==null||w.addEventListener("click",()=>j==null?void 0:j.click());const K=h=>{if(!h)return;const $=new FileReader;$.onload=B=>{a=B.target.result,R&&(R.src=a),w&&(w.style.borderColor="var(--primary-color)",w.innerHTML=`
          ...
          `)},$.readAsDataURL(h)};j==null||j.addEventListener("change",h=>K(h.target.files[0])),w==null||w.addEventListener("dragover",h=>{h.preventDefault(),w&&(w.style.borderColor="var(--primary-color)")}),w==null||w.addEventListener("dragleave",()=>{w&&(w.style.borderColor="var(--border-color)")}),w==null||w.addEventListener("drop",h=>{h.preventDefault(),w&&(w.style.borderColor="var(--border-color)"),K(h.dataTransfer.files[0])}),w&&(w.setAttribute("tabindex","0"),w.addEventListener("paste",h=>{var B;const $=(B=h.clipboardData)==null?void 0:B.items;if($){for(let S=0;S<$.length;S++)if($[S].type.indexOf("image")!==-1){h.preventDefault(),K($[S].getAsFile());break}}})),Y==null||Y.addEventListener("input",h=>{const $=h.target.value.trim();$&&(a=$,R&&(R.src=a))});const G=document.getElementById("editor-write-tab"),tt=document.getElementById("editor-preview-tab"),rt=document.getElementById("editor-textarea-wrapper"),it=document.getElementById("editor-preview-wrapper");G==null||G.addEventListener("click",()=>{G.classList.add("active"),tt==null||tt.classList.remove("active"),rt&&(rt.style.display="block"),it&&(it.style.display="none")}),tt==null||tt.addEventListener("click",()=>{if(tt.classList.add("active"),G==null||G.classList.remove("active"),rt&&(rt.style.display="none"),it){it.style.display="block";const h=(u==null?void 0:u.innerHTML)||"";it.innerHTML=h?Rt(h):'<p style="color:var(--text-muted); font-style:italic;">Chưa soạn thảo nội dung...</p>'}}),C==null||C.addEventListener("click",()=>{const h=r==null?void 0:r.value.trim(),$=d==null?void 0:d.value,B=(p==null?void 0:p.value)||"",S=f==null?void 0:f.value,ot=T==null?void 0:T.value.trim(),gt=u==null?void 0:u.innerHTML.trim(),yt=m==null?void 0:m.checked;if(!h||!$||!S||!ot||!gt){alert("Vui lòng điền đầy đủ thông tin bắt buộc (*).");return}C.disabled=!0,C.textContent="⏳ Đang xuất bản...",setTimeout(()=>{const bt=st();if(s){const U=i.find(ht=>ht.id===s);U&&(U.title=h,U.category=$,U.subcategory=B,U.image=a,U.author=S,U.description=ot,U.content=gt,U.featured=yt,U.readTime=bt),localStorage.setItem("toptech_articles",JSON.stringify(i))}else{const ht=[{id:Date.now(),title:h,category:$,subcategory:B,image:a,author:S,date:"Vừa đăng",readTime:bt,views:0,comments:0,featured:yt,description:ot,content:gt,commentsList:[]},...i];localStorage.setItem("toptech_articles",JSON.stringify(ht)),i.splice(0,i.length,...ht)}o(),C.style.backgroundColor="#10b981",C.style.borderColor="#10b981",C.innerHTML="✓ Lưu bài viết thành công!",setTimeout(()=>{c="list",v()},800)},800)})}function Q(){const r=H(),d=N();return`
      <div class="create-post-layout-grid">
    ...
    `}function J(){const r=document.getElementById("input-cat-name"),d=document.getElementById("btn-save-category"),p=document.getElementById("btn-cancel-edit-category");d==null||d.addEventListener("click",()=>{const m=r==null?void 0:r.value.trim();if(!m){alert("Vui lòng nhập tên chuyên mục.");return}let f=H(),T=N();if(n){const u=f.indexOf(n);u!==-1&&(f[u]=m,T[n]&&(T[m]=T[n],delete T[n]),i.forEach(C=>{C.category===n&&(C.category=m)}),localStorage.setItem("toptech_articles",JSON.stringify(i))),n=null}else{if(f.includes(m)){alert("Chuyên mục đã tồn tại!");return}f.push(m),T[m]=[]}q(f),x(T),o(),v()}),p==null||p.addEventListener("click",()=>{n=null,v()}),e.querySelectorAll(".edit-cat-btn").forEach(m=>{m.addEventListener("click",()=>{n=m.dataset.cat,v()})}),e.querySelectorAll(".delete-cat-btn").forEach(m=>{m.addEventListener("click",()=>{const f=m.dataset.cat;if(confirm(`Xóa chuyên mục "${f}" sẽ xóa toàn bộ các chuyên mục con tương ứng. Tiếp tục?`)){let T=H().filter(C=>C!==f),u=N();delete u[f],q(T),x(u),o(),v()}})}),e.querySelectorAll(".btn-add-subcategory").forEach(m=>{m.addEventListener("click",()=>{const f=m.dataset.cat,T=e.querySelector(`.inline-sub-input[data-cat="${f}"]`),u=T==null?void 0:T.value.trim();if(!u){alert("Vui lòng nhập tên chuyên mục con.");return}let C=N();if(C[f]||(C[f]=[]),C[f].includes(u)){alert("Chuyên mục con này đã tồn tại trong chuyên mục chính!");return}C[f].push(u),x(C),v()})}),e.querySelectorAll(".delete-sub-btn").forEach(m=>{m.addEventListener("click",()=>{const f=m.dataset.cat,T=m.dataset.sub;let u=N();u[f]&&(u[f]=u[f].filter(C=>C!==T),x(u),v())})})}function Z(){const r=I(),d=y?r.find(p=>p.name===y):null;return`
      <div class="create-post-layout-grid">
    ...
    `}function ct(){const r=document.getElementById("input-author-name"),d=document.getElementById("input-author-role"),p=document.getElementById("input-author-avatar"),m=document.getElementById("input-author-desc"),f=document.getElementById("btn-save-author"),T=document.getElementById("btn-cancel-edit-author");f==null||f.addEventListener("click",()=>{const u=r==null?void 0:r.value.trim(),C=d==null?void 0:d.value.trim(),R=p==null?void 0:p.value.trim(),V=m==null?void 0:m.value.trim();if(!u||!C||!R||!V){alert("Vui lòng điền đầy đủ các thông tin bắt buộc (*).");return}let g=I();if(y){const b=g.find(L=>L.name===y);b&&(b.role=C,b.avatar=R,b.desc=V),y=null}else{if(g.find(L=>L.name.toLowerCase()===u.toLowerCase())){alert("Tên tác giả đã tồn tại trong danh sách!");return}g.push({name:u,role:C,avatar:R,desc:V})}P(g),o(),v()}),T==null||T.addEventListener("click",()=>{y=null,v()}),e.querySelectorAll(".edit-author-btn").forEach(u=>{u.addEventListener("click",()=>{y=u.dataset.name,v()})}),e.querySelectorAll(".delete-author-btn").forEach(u=>{u.addEventListener("click",()=>{const C=u.dataset.name;if(confirm(`Bạn có chắc chắn muốn xóa tác giả "${C}"?`)){let R=I().filter(V=>V.name!==C);P(R),o(),v()}})})}v()}const Vt={AI:"https://images.unsplash.com/photo-1677442136019-21780efad99a?auto=format&fit=crop&q=80&w=600&h=400","Tin công nghệ":"https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&q=80&w=600&h=400","Chuyển đổi số":"https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=600&h=400",Review:"https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?auto=format&fit=crop&q=80&w=600&h=400","Phần mềm":"https://images.unsplash.com/photo-1607799279861-4dd421887fb3?auto=format&fit=crop&q=80&w=600&h=400",IT:"https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&q=80&w=600&h=400","Giải pháp công nghệ":"https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80&w=600&h=400",Casestudy:"https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&q=80&w=600&h=400"};function _t(e,i,l){const o=l.map(t=>{const c=i.filter(a=>a.category===t).length,s=Vt[t]||"https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&q=80&w=600&h=400";return`
  ...
  `}function Ft(e,i,l){const o=ft[i]||{avatar:"https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=80&h=80",role:"Cộng tác viên",desc:"Tác giả chia sẻ các bài viết kiến thức hữu ích tại TopTech."},t=l.filter(s=>s.author===i),c=t.length===0?`
